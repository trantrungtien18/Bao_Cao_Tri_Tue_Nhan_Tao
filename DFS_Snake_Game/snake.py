...
class Square:
    ...
    def draw(self, clr=SNAKE_CLR):
        ...
        if self.dir == [-1, 0]:
            if self.is_tail:
                pygame.draw.rect(self.surface, clr, (x * ss +
                                 gs, y * ss + gs, ss - 2*gs, ss - 2*gs))
            else:
        ...
        if self.is_apple:
            pygame.draw.rect(self.surface, clr, (x * ss + gs,
                             y * ss + gs, ss - 2*gs, ss - 2*gs))
...
class Snake:
    def __init__(self, surface):
        self.surface = surface
        self.is_dead = True
        self.squares_start_pos = [[ROWS // 2 + i, ROWS // 2]
                                  for i in range(INITIAL_SNAKE_LENGTH)]
        self.turns = {}
        self.dir = [-1, 0]
        self.score = 0
        self.moves_without_eating = 0
        self.apple = Square([randrange(ROWS), randrange(ROWS)],
                            self.surface, is_apple=True)
    ...
    def add_square(self):
        ...
        if direction == [0, 1]:
            self.squares.append(
                Square([tail.pos[0], tail.pos[1] - 1], self.surface))
        if direction == [0, -1]:
            self.squares.append(
                Square([tail.pos[0], tail.pos[1] + 1], self.surface))
    ...
    def dfs(self, s, e):  # Find path between (start_position, end_position)
        stack = [s]  # Stack
    ...
    def set_path(self):
        ...
        if path_2:  # If there is a path between v_snake and it's tail
            # Choose BFS path to apple (Fastest and shortest path)
            return path_1
